...
namespace SMARTscan_DataProcessor.Controller
{
    public class MailSender
    {
        ...
        public static void SendNotificationWithPath(string jobname, string path)
        {
            string carnellSMTP = Global.SmtpServer;
            SmtpClient SmtpServer = new SmtpClient(carnellSMTP);
            string time = DateTime.Now.ToString("dddd, dd MMMM yyyy HH:mm:ss");
            ...
            mail.To.Add(Global.Hao);
            mail.Subject = "SMARTscan Automation Email - Status: Ready for QA";
            mail.Body = $"<h3>This is an automated email, please do not reply.</h3><p>Your job has been successfully processed and need you to implement data QA.</p><p> Date: {time} </p><p> Job Number: {jobname} </p><a href={@"file:///"+path}>ClickPath</a>  <p> Thank you for uploading the data!</p> ";
            mail.IsBodyHtml = true;
